...
class cookie
{
  ...
  {
    return (bool)empty($_COOKIE[$name]);
  }
   ...
   * @return bool
   */
  static public function write($name, $value, $expiry = self::Forever, $path = '/', $domain = NULL, $secure=0)
  {
    ...
    if (!headers_sent())
    {
      ...
      $retval = @setcookie($name, $value, $expiry, $path, $domain,$secure);
      if ($retval)
        $_COOKIE[$name] = $value;
    }
    return $retval;
